...
| EWP diagrams (scoped styles, drawn in code; status swatches are the product's own colours, shown as a specimen) | `components/ewp/`: `ModuleMatrix`, `StatusSystem`, `OldNew` (real old-UI fragments at near-true size), `TalkLink` (the recording can't be embedded) | EWP only |
...
| Section trail ("where am I", in the site header: object and current case or step after the name, and a progress line along the header's bottom edge, one segment per section, filling in the object's form colour; nothing appears or moves) | `SectionTrail`, `rail` frontmatter: `cases` (Ericsson letters), `call` (DBAS steps with a live state), `status` (EWP) | every case study |
